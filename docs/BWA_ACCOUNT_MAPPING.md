...
Ported from a sister Hub's equivalent feature (`docs/BWA_ACCOUNT_MAPPING.md` there). Records what's
...
`bwa_account_mapping` (migration `0043`) was seeded empty and, unlike a sister Hub, **has no data at
all yet for any company** — there is no equivalent of a sister Hub's IMKO/IMGM chart-of-accounts
...
- `src/lib/data/queries.ts` — `useBwaAccountMapping(fiscalYear, companyId)` requires a company
  ...
  `companyId`.
- `KontenrahmenTab` — company `Combobox` (no default company preferred, unlike a sister Hub's IMKO
...
- No business-line dimension anywhere in this repo (`useScopeChips`/`RegelZeile` has no
  `business_line_id` chip) — this client Hub replaced that model with direct property↔company assignment
...
Same design as a sister Hub: the old manual "Konto;Kategorie-Code" textarea import
...
- `src/lib/api/chart-of-accounts-extraction-shared.ts` — allowed file-mode MIME types and
  size/char ceilings, identical to a sister Hub's.
- `src/lib/api/chart-of-accounts-extraction.functions.ts` — `extractChartOfAccounts` server
  function. Same shape as a sister Hub's, adapted to this repo's own error class
  (`OpenAiApiError` from `./errors`, not a sister Hub's `AiExtractionError` — this repo has no such
...
- `src/components/zuordnung/ki-import-kontenrahmen-dialog.tsx` — `KiImportKontenrahmenDialog`,
  copied verbatim from a sister Hub (no a sister Hub-specific dependencies in that file). XLSX/XLS are
...
The underlying OpenAI Responses API call/schema is identical to a sister Hub's, which was verified
end-to-end against the live API (2026-08-07) with a synthetic 3-row CSV — see a sister Hub's own doc
...
- **No company has any chart-of-accounts data yet.** Unlike a sister Hub, there is no IMKO/IMGM-style
...
- **RLS is unchanged** — this screen has no role gating today (any authenticated user can
  view/import for any company), consistent with a sister Hub's equivalent note.

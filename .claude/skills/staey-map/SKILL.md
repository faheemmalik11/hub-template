---
name: staey-map
description: Architecture, navigation, and file map for the Stäy Hub codebase. Use FIRST for any task in this repo — to understand where code lives, how the layers connect, how data flows browser→Supabase, what's real vs placeholder, and which doc answers a question. Load before editing so you don't re-read the whole tree.
---

# Stäy Hub — codebase map
...
`../book-keeping` locally), in which staeyhub is one tenant, with behaviour picked by one
...
4 FEATURE src/components/belege/*, /brand/             ← project pieces (design lives here)
...
| `/lieferanten`, `/lieferanten/$id` | `lieferanten/*`                 | real — supplier CRUD (soft-delete)                      |
...
| `/protokoll`                       | `protokoll/index.tsx`           | real — reads `verarbeitungs_log` (pipeline output)      |
...
- `docs/PIPELINE_STAEY.md` — the superseded in-repo pipeline (`pipeline_new/`), context only.
...
- `staey-instructions` — standing team rules to follow on EVERY task (load at the start).
- `staey-data` — how to add/change Supabase reads & mutations.
- `staey-ui` — routes, badges, formatting, German/English rule, the 3 status axes.
- `staey-gotchas` — known bugs, hardcoded values, footguns before you edit.

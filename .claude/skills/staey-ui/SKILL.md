---
name: staey-ui
description: UI conventions for the Stäy Hub — adding routes/pages, the English-code/German-UI language rule, formatting helpers, badges/chips, loading & error states, and the three overlapping status axes (status / workflow_status / bezahlt_am). Use when building or editing any page, label, badge, or user-facing text.
---
...
export const Route = createFileRoute("/eingangsrechnungen/upload")({
  head: () => ({ meta: [{ title: "Beleg hochladen — Stäy" }] }),
...
## Badges & chips (`src/components/belege/badges.tsx`) — colors/labels come from format.ts
...
See `staey-gotchas` for stale hardcoded values (2026 date presets, etc.) and `staey-data` for hooks.

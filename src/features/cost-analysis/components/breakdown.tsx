...
const ROWS_STYLE: Record<string, { icon: typeof Plane; tile: string; text: string; bar: string }> =
  {
    advertising_travel: {
      icon: Plane,
      tile: "bg-violet-100",
...
export function EntriesTable({
  ...
  const { visible, sentinel, hatMore, rest } = useInfiniteRows(items, FIRST_PAGE, LOADMORE);
        ...
        <thead className="sticky top-0 bg-card">
          <tr className="text-left text-muted-foreground">
            <th className="py-2 pl-3 pr-3 font-normal">
              {t("reports.aufschluesselung.spalteBeleg")}
            </th>
            {categoryFor && (
              <th className="py-2 pr-2 font-normal">{t("reports.filter.labelKategorie")}</th>
            )}
            <th className="py-2 pr-2 font-normal">{t("reports.aufschluesselung.spalteDatum")}</th>
            <th className="py-2 pl-2 pr-3 text-right font-normal">
              {t("reports.aufschluesselung.spalteBetrag")}
            </th>
          </tr>
        </thead>
        <tbody>
          {items.slice(0, visible).map((item, idx) => {
            const clickable = !!item.documentId || !!item.manualBookingSourceId;
            const path = categoryFor?.(item) ?? null;
            const open = () => {
            ...
            return (
              <tr
                ...
                key={`${item.documentId ?? item.outgoingInvoiceId ?? item.manualBookingSourceId ?? "item"}-${idx}`}
                className={cn(
                      ...
                      <span className="inline-flex flex-wrap items-center gap-1 rounded-full bg-card px-2 py-0.5 text-xs text-muted-foreground">
                        ...
                        {path.subcategory && (
                          <>
                            ...
                            <span className="text-foreground">{path.subcategory}</span>
                          </>
                        )}
                      </span>
                    )}
          ...
          {hatMore && (
            <tr ref={sentinel as React.RefObject<HTMLTableRowElement>}>
              <td colSpan={categoryFor ? 4 : 3} className="py-2 text-sm text-muted-foreground">
                {t("reports.belegListe.weitere", { count: rest })}
              </td>
            </tr>
          )}
        </tbody>
...
function GroupsTable({
  ...
  totalCost: number;
  laden?: boolean;
  onOpen: (row: BreakdownRow) => void;
}) {
  const { t } = useTranslation();
  const countTotal = group.rows.reduce((n, z) => n + z.items.length, 0);
  ...
  return (
    ...
    <div className="flex h-full min-w-0 flex-col">
      <h3 className="flex flex-wrap items-baseline gap-x-2 text-base font-semibold text-foreground">
        {group.label}
        {group.subtitle && (
          <span className="text-sm font-normal text-muted-foreground">{group.subtitle}</span>
        )}
      </h3>
        ...
        <thead>
          <tr className="text-left text-muted-foreground">
            <th className="py-2 pl-3 pr-2 font-normal">
              {t("reports.aufschluesselung.spalteKategorie")}
            </th>
            <th className="py-2 pl-2 pr-3 text-right font-normal sm:pr-8">
              {t("reports.spalte.betrag")}
            </th>
            <th className="py-2 pl-2 text-left font-normal">
              {t("reports.aufschluesselung.spalteAnteil")}
            </th>
            <th className="py-2 pl-2 pr-3 text-right font-normal">
              {t("reports.aufschluesselung.spalteAnzahl")}
            </th>
          </tr>
        </thead>
        <tbody>
          ...
          {group.rows.map((z) => {
            const share = totalCost > 0 ? (z.amount / totalCost) * 100 : null;
            const style =
              ROWS_STYLE[z.key] ??
              (group.styleKey ? ROWS_STYLE[group.styleKey] : undefined) ??
              STANDARD_STYLE;
            ...
            const clickable = !laden && z.items.length > 0;
            return (
              <tr
                key={z.key}
                role={clickable ? "button" : undefined}
                tabIndex={clickable ? 0 : undefined}
                onClick={clickable ? () => onOpen(z) : undefined}
                onKeyDown={
                  clickable
                    ? (e) => {
                        if (e.key === "Enter" || e.key === " ") {
                ...
                className={cn(
                  ...
                  clickable ? "cursor-pointer" : "text-muted-foreground",
                )}
              >
                <td className={cn("py-2 pl-3 pr-2", clickable && HOVER, "rounded-l-md")}>
                  <span className="flex items-center gap-2">
                    <span
                      className={cn(
                    ...
                    <span className={cn("min-w-0", clickable && "text-foreground")}>{z.label}</span>
                  </span>
                </td>
                <td
                  className={cn(
                ...
                <td className={cn("py-2 pl-2 text-muted-foreground", clickable && HOVER)}>
                  {share == null ? (
                    "—"
                  ) : (
        ...
        <tbody>
          <tr className="border-t border-foreground/25 font-medium">
            <td className="py-2 pl-3 pr-2 text-foreground">
              {t("reports.aufschluesselung.summe", { group: group.label })}
            </td>
            <td className="py-2 pl-2 pr-3 text-right tabular-nums text-foreground sm:pr-8">
              {laden ? <Skeleton className="ml-auto h-4 w-24" /> : formatEUR(group.total)}
            </td>
            <td className="py-2 pl-2 text-muted-foreground">
              <span className="flex items-center gap-2.5">
                ...
                <span className="w-14 shrink-0 text-right tabular-nums">
                  {totalCost > 0
                    ? `${((group.total / totalCost) * 100).toLocaleString("de-DE", {
...
export function Breakdown({
  ...
  const totalCost = groups.reduce((n, g) => n + g.total, 0);
  const style = (open && ROWS_STYLE[open.key]) || STANDARD_STYLE;
  const OpenIcon = style.icon;

  if (groups.length === 0 && !laden) return null;
    ...
    <section className="mt-8">
      ...
      <div className="flex flex-wrap items-start justify-between gap-x-8 gap-y-3">
        <div className="min-w-0">
          <h2 className="text-base font-semibold tracking-tight text-foreground">
            {t("reports.tabelle.titel")}
          </h2>
          <p className="mt-0.5 text-sm text-muted-foreground">
            {t("reports.aufschluesselung.untertitel")}
          </p>
        </div>
        {hint && <div className="min-w-0 lg:max-w-[60%]">{hint}</div>}
      </div>

      <div className="mt-4 grid gap-x-8 gap-y-6 lg:grid-cols-2">
        {groups.map((g) => (
          <GroupsTable key={g.key} group={g} totalCost={totalCost} onOpen={setOpen} />
        ))}
      </div>
      ...
      {groups.length > 1 && (
        <p className="mt-4 flex items-baseline justify-end gap-3 border-t border-border pt-3 text-sm text-muted-foreground">
          <span>{t("reports.aufschluesselung.gesamtkosten")}</span>
        ...
        <DialogContent className="max-w-3xl gap-0 p-6">
          <DialogHeader className="pb-4">
            <DialogTitle className="flex flex-wrap items-center gap-x-3 gap-y-2 pr-8 text-left">
              ...
              >
                <OpenIcon className={cn("size-4", style.text)} />
              </span>
              <span className="text-base font-semibold text-foreground">{open?.label}</span>
              <span
                className="text-xl font-semibold tabular-nums text-foreground"
                title={t("reports.aufschluesselung.gesamtbetrag")}
              >
                {formatEUR(open?.amount ?? 0)}
              </span>
              <span className="inline-flex items-center gap-1.5 rounded-md bg-muted px-2.5 py-1 text-sm font-normal text-muted-foreground">
                ...
                {t("reports.aufschluesselung.anzahl", { count: open?.items.length ?? 0 })}
              </span>
            </DialogTitle>
          </DialogHeader>

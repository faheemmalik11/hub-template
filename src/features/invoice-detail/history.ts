...
import type { DocumentHistory } from "@/lib/data/types";

import { ACTION_TARGETSTATUS, APPROVAL_HISTORY_TYPES, LEGACY_ACTION_IDS } from "./config";

// `t` is passed in so this stays a plain helper (no hook) while still translating.
export function historyTypeLabel(type: string, t: (key: string) => string): string {
  const known = [
    "note",
    "status_change",
    "change",
  ...
  return known.includes(type) ? t(`documents.detail.verlaufTyp.${type}`) : type;
}

/** The action a legacy row recorded, read back out of its German audit sentence. */
function legacyActionAusText(text: string): ApprovalActionId | null {
...
function legacyCorrectionTarget(text: string): string | null {
  const match = /^Manuell korrigiert:\s*.+?\s*→\s*(.+)$/.exec(text.trim());
  if (!match) return null;
  return WORKFLOW_ORDER.find((s) => workflowLabelDe(s) === match[1].trim()) ?? null;
}
 ...
 * what somebody typed.
 */
export function historyComment(v: DocumentHistory): string | null {
  const saved = v.data?.kommentar;
  ...
  const reason = (v.data as { grund?: unknown } | null)?.grund;
  ...
  if (!text) return null;
  const id = legacyActionAusText(text);
  if (!id) return null;
  const de = approvalActionLabelDe(id);
 ...
 */
export function historyTargetStatus(v: DocumentHistory): string | null {
  ...
  const aus = v.text ? legacyActionAusText(v.text.trim()) : null;
  return aus ? (ACTION_TARGETSTATUS[aus] ?? null) : legacyCorrectionTarget(v.text ?? "");
}
...
export function historyBasis(
  ...
  const target = historyTargetStatus(v);
  return target
    ? t(`documents.detail.historie.${target}`, {
        defaultValue: t(`documents.workflow.${target}`, { defaultValue: target }),
      })
    : historyTypeLabel(v.type, t);
}

/** How the invoice got there, when it was not a plain step along the chain. Null when it was. */
export function historyExtra(
  v: DocumentHistory,
  t: (key: string, opts?: Record<string, unknown>) => string,
): string | null {
  if (v.type === "correction") return t("documents.detail.historie.korrigiert");
  ...
  if (v.type === "zuordnung_getrennt") return t("documents.detail.historie.zuordnungGetrennt");
  if (v.type === "zuordnung_bestaetigt") return t("documents.detail.historie.zuordnungBestaetigt");
  if (v.type === "query") return t("documents.detail.historie.rueckfrageZusatz");
  ...
  if (v.type === "paid") {
    const source = (v.data as { paid_source?: unknown } | null)?.paid_source;
    if (typeof source === "string" && source.trim() !== "") {
      return t(`documents.detail.historie.bezahltVia.${source}`, {
        defaultValue: t("documents.detail.historie.bezahltVia.manual"),
      });
    }
  }
  return null;
...
export function historyRows(
  ...
  if (typeof event === "string" && event !== "") {
    ...
    if (event === "paid_from_match") {
      return [
        d.cashDiscount != null
          ? t("documents.detail.historie.event.paidFromMatchSkonto", {
              cashDiscount: asNumber(d.cashDiscount),
            })
          : t("documents.detail.historie.event.paidFromMatch"),
      ];
    }
    if (event === "match_confirmed") return [t("documents.detail.historie.event.matchConfirmed")];
    if (event === "match_rejected") {
      const reason = historyComment(v);
      return [
        reason
          ? t("documents.detail.historie.event.matchRejectedGrund", { reason })
          : t("documents.detail.historie.event.matchRejected"),
      ];
    }
    if (event === "match_unlinked") {
      ...
      return [
        reason
          ? t("documents.detail.historie.event.matchUnlinkedGrund", { reason })
          : t("documents.detail.historie.event.matchUnlinked"),
      ];
    }
    if (event === "remainder_reopened") {
      return [t("documents.detail.historie.event.remainderReopened")];
    }
    if (event === "remainder_written_off") {
      const reason = historyComment(v);
      return [
        reason
          ? t("documents.detail.historie.event.remainderWrittenOffGrund", { reason })
          : t("documents.detail.historie.event.remainderWrittenOff"),
      ];
    }
    if (event === "payment_withdrawn") {
      return [
        t("documents.detail.historie.event.paymentWithdrawn", {
          matched: asNumber(d.matched),
          gross: asNumber(d.gross),
        }),
      ];
    }
  }
  ...
  if (
    historyTargetStatus(v) !== null ||
    v.type === "correction" ||
    v.type === "zuordnung_getrennt" ||
    v.type === "zuordnung_bestaetigt" ||
    v.type === "query"
  ) {
    const comment = historyComment(v);

...
import { formatDateTime, formatDuration } from "@/lib/data/format";
import type { DocumentHistory } from "@/lib/data/types";

import { historyComment } from "./history";
import { QUERY_ANCHOR } from "@/features/invoice-detail/config";
  ...
  return (
    <ol>
      {display.map(({ v, nach, extra, ms }, i) => {
        ...
        const actingAls = (v.data as { handelnd_als?: string | null } | null)?.handelnd_als;
        const comment = historyComment(v);
        const bad = v.type === "rejection" || v.type === "payment_failed";
        ...
        return (
          <li
            ...
            id={latest && v.type === "query" ? QUERY_ANCHOR : undefined}
          ...
          >
            ...
            <div className="flex w-12 shrink-0 flex-col items-end pr-1">
              ...
              {duration.value > 0 && (
                <span className="flex flex-1 items-center whitespace-nowrap text-xs tabular-nums text-muted-foreground">
                  {t(`documents.detail.workflow.dauerKurz.${duration.unit}`, {
            ...
            <div className="flex w-5 shrink-0 flex-col items-center">
              <span
                className={cn(
                  ...
                  latest && "ring-2 ring-brand/60 ring-offset-2 ring-offset-card",
                )}
              />
              <ArrowUp
            ...
            <div className="min-w-0 pb-5">
                  ...
                  {!extra && comment && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        ...
                        >
                          {t("documents.detail.workflow.grund")}
                          {/* A visible cue that the chip carries text. Without it the tooltip was
                              undiscoverable: nothing on the row said there was a reason to read. */}
                          <Info className="size-3 shrink-0 opacity-70" aria-hidden />
                        </span>
              ...
              <p className="mt-0.5 text-sm text-muted-foreground">
                ...
                {actingAls &&
                  ` · ${t("documents.detail.workflow.handelndAls", { name: actingAls })}`}{" "}
        ...
        <li className="flex gap-2">
          ...
          <div className="min-w-0">
            <p className="text-base font-medium text-foreground">
              {t("documents.detail.historie.eingegangen")}
            </p>
            <p className="mt-0.5 text-sm text-muted-foreground">{formatDateTime(receivedAm)}</p>
          </div>
        </li>
      )}
    </ol>

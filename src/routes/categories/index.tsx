...
import {
  ...
  type CostAnalysisCategoryInput,
} from "@/data";
...
function CategoriesTab() {
  ...
  const childrenOf = useMemo(() => {
    const m = new Map<string, CostAnalysisCategory[]>();
  ...
  const match = useMemo(() => {
    const matches = (c: CostAnalysisCategory) =>
      `${c.name} ${c.note ?? ""}`.toLowerCase().includes(q);
    const out: { group: CostAnalysisCategory; kids: CostAnalysisCategory[]; onlyKind: boolean }[] =
      [];
    for (const group of coarse) {
      const kids = childrenOf.get(group.id) ?? [];
      if (!q || matches(group)) {
        out.push({ group, kids, onlyKind: false });
        continue;
      }
      const kinder = kids.filter(matches);
      if (kinder.length > 0) out.push({ group, kids: kinder, onlyKind: true });
    }
    return out;
  }, [coarse, childrenOf, q]);
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <p className="max-w-2xl text-sm text-muted-foreground">{t("categories.hinweis")}</p>
      ...
      <div className="relative mt-4 w-full max-w-sm">
        ...
        <Input
          ...
          placeholder={t("categories.suche")}
          className="pl-9"
        />
      </div>
      ...
      {categoriesQ.isError ? (
        ...
        <>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t("categories.col.hinweis")}</TableHead>
                  <TableHead className="text-right">{t("categories.col.aktionen")}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {match.map(({ group, kids, onlyKind }) => {
                  ...
                  const isOpen = expanded.has(group.id) || onlyKind;
                  return (
                      ...
                      <TableRow className={cn(!group.is_active && "opacity-60")}>
                        ...
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <NewCategoryDialog categories={categories} presetParentId={group.id} />
                            <CategoryActions category={group} />
                          </div>
                        </TableCell>
                      </TableRow>
                      {isOpen
          ...
          {match.length === 0 ? (
            <div className="mt-6">
              <EmptyState
                title={t("categories.keineTreffer")}
                hint={t("categories.keineTrefferHint")}
              />
            </div>
          ) : null}
        </>
...
function CategoryActions({ category }: { category: CostAnalysisCategory }) {
  const { t } = useTranslation();
  const update = useUpdateCostAnalysisCategory();
  const remove = useSoftDeleteCostAnalysisCategory();
  ...
  return (
    <div className="flex justify-end gap-1">
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogTrigger asChild>
          <Button variant="ghost" size="sm">
            {t("categories.action.bearbeiten")}
          </Button>
        </DialogTrigger>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <Label>{t("categories.feld.name")}</Label>
              <Input value={name} onChange={(e) => setName(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label>{t("categories.feld.notiz")}</Label>
            ...
            <Button
              ...
              onClick={() =>
                update.mutate(
                  ...
                  {
                    onSuccess: () => {
                      toast.success(t("categories.toast.gespeichert"));
                      setEditOpen(false);
                    },
                    onError: (e) =>
                      toast.error(
      ...
      <AlertDialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("categories.loeschen.title")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("categories.loeschen.desc", { name: category.name })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <Input
            value={reason}
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() =>
                remove.mutate(
                  { id: category.id, reason: reason.trim() },
                  {
                    onSuccess: () => toast.success(t("categories.toast.geloescht")),
                    onError: (e) =>
                      toast.error(
                        t("assignmentRules.toast.fehlgeschlagen", {
                          error: errorText(e),
...
function NewCategoryDialog({
  ...
  const [direction, setDirection] = useState<CostAnalysisCategory["direction"]>("outgoing");
  const create = useCreateCostAnalysisCategory();
  ...
  function save() {
    ...
    const derivedBlock: CostAnalysisCategory["report_block"] =
      direction === "incoming" ? "einnahmen" : "kosten";
    const input: CostAnalysisCategoryInput = {
    ...
    if (!input.name) {
      toast.error(t("categories.neu.wertFehlt"));
      return;
    }
    create.mutate(input, {
      onSuccess: () => {
        toast.success(t("categories.toast.angelegt"));
        reset();
        setOpen(false);
      },
      onError: (e) => {
        ...
        toast.error(
          /duplicate key|bwa_categories_code_uniq/i.test(msg)
            ? t("categories.neu.codeSchonVorhanden")
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {presetParentId ? (
          <Button
            ...
            size="icon"
            className="size-8"
            title={t("categories.neu.unterkategorie")}
            aria-label={t("categories.neu.unterkategorie")}
          >
            <Plus className="size-4" />
          </Button>
        ) : (
          <Button className="gap-2">
            <Plus className="size-4" /> {t("categories.neu.button")}
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>
            {presetParentId ? t("categories.neu.unterkategorie") : t("categories.neu.title")}
          </DialogTitle>
          <DialogDescription>{t("categories.neu.desc")}</DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          {presetParentId ? (
            <p className="text-xs text-muted-foreground">
              {t("categories.feld.unterVon", { name: parent?.name ?? "" })}
            </p>
          ) : (
            <div className="space-y-1.5">
              <Label>{t("categories.feld.parent")}</Label>
              <Combobox
                value={parentId === LEER ? null : parentId}
                onValueChange={setParentId}
                options={[
                  { value: LEER, label: t("categories.feld.parentKeiner") },
                  ...coarseOptions,
                ]}
              />
            </div>
          ...
          {!presetParentId && parentId === LEER && (
            <div className="space-y-1.5">
              <Label>{t("categories.feld.art")}</Label>
              <Combobox
                value={direction}
                onValueChange={(v) => setDirection(v as CostAnalysisCategory["direction"])}
                options={[
                  { value: "outgoing", label: t("categories.block.kosten") },
                  { value: "incoming", label: t("categories.block.einnahmen") },
                ]}
              />
            </div>
          )}

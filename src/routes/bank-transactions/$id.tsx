...
import { DirectionBadge, TransactionTypeBadge, TxnMatchingBadge } from "@/components/bank/badges";
...
import { NotifySomeone, PingNotice } from "@/components/documents/ping-button";
import { ErrorState } from "@/components/documents/query-states";
import { useTranslation } from "@/lib/i18n";
import { pageTitle } from "@/config/brand";

export const Route = createFileRoute("/bank-transactions/$id")({
...
function TransactionDetailPage() {
  ...
  const account = (accountsQ.data ?? []).find((a) => a.id === txn?.account_id) ?? null;
  const company = (companiesQ.data ?? []).find((g) => g.id === txn?.company_id) ?? null;
  ...
  const nothingMoreOpen = fullUsed || (assigned > 0 && rest <= 0.01);
  ...
  useEffect(() => {
    if (!txn) return;
    const who = txn.counterparty_holder?.trim() || formatDate(txn.booking_date);
    document.title = pageTitle(`${who} · ${formatSignedEUR(txn.amount)}`);
  }, [txn]);
    ...
    return (
      <div className="mx-auto max-w-md py-20 text-center">
        ...
        <Button asChild className="mt-6">
          <Link to="/bank-transactions">{t("bank.detail.toOverview")}</Link>
        </Button>
      </div>
    );
  }
      ...
      {txn.matching_status === "ignored" && txn.no_receipt_reason && (
        <p className="mt-3 text-sm text-muted-foreground">
          {t("noReceipt.detail.grund", {
            reason: t(`openItemWhitelist.category.${txn.no_receipt_reason}`),
          })}{" "}
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(320px,420px)_1fr]">
        ...
        >
          ...
          <Row
            ...
            value={account ? (account.account_name ?? account.iban ?? "—") : "—"}
          />
          {account?.iban && (
            <Row label={t("bank.detail.row.kontoIban")} value={formatIBAN(account.iban)} mono />
          )}
              ...
              {!nothingMoreOpen && (
                <>
                  <Button asChild variant="outline" size="sm">
                    <Link to="/incoming-invoices/upload" search={{ for: `txn:${txn.id}` }}>
                      <Upload className="mr-1.5 size-4" />
                      {t("bank.detail.belege.hochladen")}
                    </Link>
                  </Button>
                  <Button asChild variant="outline" size="sm">
                    <Link to="/open-items" search={{ match: `txn:${txn.id}` }}>
              ...
              <span>
                {fullUsed
                  ...
                  : /* Unsigned: this is what is LEFT of the payment, not a movement, and
                    ...
                    t("bank.detail.restschliessen.offenZahlung", { amount: formatEUR(rest) })}
              </span>
              {fullUsed ? (
                canPay && (
                  <Button
                ...
                <CloseRemainderButton
                  ...
                  disabledReason={canPay ? null : t("bank.detail.restschliessen.keineBerechtigung")}
...
function TransactionDocumentCard({ txn }: { txn: BankTransaction }) {
  ...
  const hatDocument = (docsQ.data ?? []).length > 0;
  const documentExpected = txn.source === "pleo";
  if (!hatDocument && !documentExpected) return null;
  return (
    ...
    <div className="mb-4 space-y-2">
      {uploads.map((u) => (
        ...
        >
          ...
          >
            <Link to="/incoming-invoices/$nr" params={{ nr: u.id }}>
...
function CategoryRow({ txn }: { txn: BankTransaction }) {
  const { t } = useTranslation();
  const categoriesQ = useCostAnalysisCategories();
  const options = useCategoryOptions(categoriesQ.data ?? []);
  const setCategory = useSetTransactionCategory();
  const isCreditNote = txn.amount >= 0;
  const category = (categoriesQ.data ?? []).find((c) => c.id === txn.category_id) ?? null;
  ...
  if (isCreditNote) {
    return (
      <Row
        ...
        value={category ? category.name : t("bank.detail.kategorieGutschrift")}
      />
    );
  }
  ...
  return (
    <div className="mt-2 space-y-1.5">
      ...
      <Combobox
        ...
        onValueChange={(v) =>
          setCategory.mutate(
            { transactionId: txn.id, categoryId: v === NO_CATEGORY ? null : v },
            {
              onSuccess: () => toast.success(t("bank.detail.kategorieGespeichert")),
              onError: (e) =>
                toast.error(t("bank.detail.kategorieFehlgeschlagen", { error: errorText(e) })),
            },
          )
        }
        options={[{ value: NO_CATEGORY, label: t("bank.detail.kategorieOhne") }, ...options]}
        className="w-full"
      />
    </div>
  );

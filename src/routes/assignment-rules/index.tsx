...
import { KiImportChartOfAccountsDialog } from "@/components/assignment/ai-import-chart-of-accounts-dialog";
...
export const Route = createFileRoute("/assignment-rules/")({
...
function AssignmentRulesPage() {
  ...
  useEffect(() => {
    document.title = pageTitle(t(`assignmentRules.doktitel.${tab}`));
  }, [t, tab]);
  ...
  const panel = (value: Tab, content: ReactNode) => (
    <TabsContent
      value={value}
  ...
  return (
    <div>
      <h1 className="mt-2 text-2xl font-semibold tracking-tight text-foreground">
        {t("assignmentRules.list.title")}
      </h1>
      <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
        {t("assignmentRules.list.subtitle")}
      </p>
        ...
        >
          {VISIBLE_TABS.map((key) => (
            <TabsTrigger key={key} value={key}>
              {t(`assignmentRules.tab.${key}`)}
            </TabsTrigger>
          ))}
        </TabsList>
        {panel(
          "regeln",
          <RulesTab template={ruleTemplate} onTemplateDone={() => setRuleTemplate(null)} />,
        )}
        {panel("spielplatz", <PlaygroundTab />)}
        {panel("kontenrahmen", <ChartOfAccountsTab />)}
        {panel(
          "suggestions",
          <SuggestionsTab
    ...
    <RuleTableRow
      ...
      onApply={() =>
        applyBulk.mutate(rule.id, {
          ...
          onError: (error) => toast.error(labels.toast.failed(errorText(error))),
        })
      }
    />
  );
...
function RulesTab({
  ...
  template: { supplierId: string; categoryId: string } | null;
  onTemplateDone: () => void;
}) {
  ...
  const [editing, setEditing] = useState<string | null>(template ? CREATING : null);
  const [deleting, setDeleting] = useState<string | null>(null);
  const [previewDraft, setPreviewDraft] = useState<AssignmentRuleDraft | null>(null);

  useEffect(() => {
    if (template) setEditing(CREATING);
  }, [template]);
  ...
  async function handleSave(draft: AssignmentRuleDraft) {
    try {
      ...
      toast.error(labels.toast.failed(errorText(error)));
    }
  }
  ...
  const seed: AssignmentRuleDraftSeed | undefined =
    editing === CREATING && template
      ? { scope: { supplier: template.supplierId }, categoryId: template.categoryId }
      : undefined;
...
function ChartOfAccountsTab() {
  ...
  const mappingQ = useCostAnalysisAccountMapping(year, companyId);
  ...
  }, [mappingQ.data, search, categoryById]);
  ...
  const view = useTableView(filtered, {
    ...
    resetKey: `${companyId ?? ""}-${year}-${search}`,
  });
  ...
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="space-y-1.5">
            <Label>{t("chartOfAccounts.jahr")}</Label>
            <Combobox
              value={String(year)}
          ...
          <div className="w-full space-y-1.5 sm:w-72">
            <Label>{t("chartOfAccounts.suche")}</Label>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
              <Input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder={t("chartOfAccounts.suchePlaceholder")}
                className="pl-9"
              />
            </div>
          </div>
        </div>
        <KiImportChartOfAccountsDialog
          companies={companies}
      ...
      {!companyId ? null : mappingQ.isError ? (
        ...
        <div className="mt-6">
          <EmptyState title={t("chartOfAccounts.empty")} hint={t("chartOfAccounts.emptyHint")} />
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="mt-6">
          <EmptyState
            title={t("chartOfAccounts.keineTreffer")}
            hint={t("chartOfAccounts.keineTrefferHint")}
          />
        </div>
      ) : (
        <>
...
function SuggestionsTab({
  ...
  onRuleCreate: (template: { supplierId: string; categoryId: string }) => void;
}) {
  const { t } = useTranslation();
  ...
  const rows = useMemo(() => {
    const label = (c: CostAnalysisCategory) => {
    ...
    for (const s of suggestionsQ.data ?? []) {
      ...
      const category = viaId ?? (viaName ? categoryById.get(viaName) : undefined);
      if (!category || category.is_catchall) continue;
      out.push({
        supplierId: s.supplier_id,
        name: supplierById.get(s.supplier_id)?.name ?? s.supplier_id,
        category,
        label: label(category),
        documents: s.receipt_count,
        total: s.total_receipts,
      });
    }
    return out;
  }, [suggestionsQ.data, categoryById, categoryIdByNameLower, supplierById]);

  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase();
    if (!q) return rows;
    return rows.filter((z) => `${z.name} ${z.label}`.toLowerCase().includes(q));
  }, [rows, search]);

  const view = useTableView(filtered, {
    sortValue: (z, key) => (key === "belege" ? z.documents : z.name),
    initialSort: "belege",
    initialDir: "desc",
    resetKey: search,
  });
  ...
  return (
    <div data-tour="assignment-suggestions">
      <SearchInput value={search} onValueChange={setSearch} placeholder={t("suggestions.suche")} />
      <p className="mt-3 max-w-2xl text-sm text-muted-foreground">{t("suggestions.hinweis")}</p>
      ...
      {suggestionsQ.isError ? (
        ...
        <div className="mt-6">
          <EmptyState title={t("suggestions.empty")} hint={t("suggestions.emptyHint")} />
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="mt-6">
          <EmptyState
            title={t("suggestions.keineTreffer")}
            hint={t("suggestions.keineTrefferHint")}
          />
        </div>
      ) : (
        <>
          <div className="mt-6 hidden overflow-hidden rounded-xl border border-border sm:block">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>{t("suggestions.col.lieferant")}</TableHead>
                  <TableHead>{t("suggestions.col.belege")}</TableHead>
                  <TableHead>{t("suggestions.col.kategorie")}</TableHead>
                  <TableHead className="text-right">{t("suggestions.col.aktion")}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {view.pageRows.map((z) => (
                  <TableRow key={z.supplierId}>
                    ...
                    <TableCell className="text-sm tabular-nums text-muted-foreground">
                      {t("suggestions.belegeCount", { n: z.documents, total: z.total })}
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <Button
                        ...
                        onClick={() =>
                          onRuleCreate({ supplierId: z.supplierId, categoryId: z.category.id })
                        }
                      >
                        {t("suggestions.action.regelAnlegen")}
                      </Button>
                    </TableCell>
                  </TableRow>
                ))}

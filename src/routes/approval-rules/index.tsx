...
import { errorText, formatDate, formatDateTime, formatEUR } from "@/lib/data/format";
...
function ApprovalRulesGuard() {
  const { ready, can } = useAuth();
  if (!ready) return null;
  if (!can(PERMISSIONS.pageApprovalRules)) return <NoAccess />;
  return <ApprovalRulesRoute />;
}

const CREATING = "__new";
const EMPTY_SCENARIO: TesterQuery = { scope: {}, amount: 0, amountText: "0" };

function ApprovalRulesRoute() {

...
export function DocumentPreview({ belegId, beleg }: { belegId: string; beleg?: Beleg }) {
  const { t } = useTranslation();
  const { data: datei, isLoading, isError, error } = useBelegDatei(belegId);
  const filenameSettingsQ = useFilenameSettings();

  const originalFilename = datei?.filename ?? `beleg-${belegId}`;
  const extension = originalFilename.includes(".") ? originalFilename.split(".").pop()! : "pdf";
  const suggestedFilename =
    beleg && filenameSettingsQ.data
      ? buildSuggestedFilename(beleg, filenameSettingsQ.data, extension)
      : null;

  const usingStorage = !!datei?.storage_bucket && !!datei?.storage_path;
  const signedQ = useInvoiceFileUrl(belegId, {
    downloadFilename: suggestedFilename ?? undefined,
    enabled: !datei || usingStorage,
  });

  // Bytea fallback — only reached for pre-cutover rows that never got a Storage copy.
  const blob = useMemo(() => {
    if (usingStorage || !datei?.content) return null;
    try {
      const bytes = hexToUint8Array(datei.content);
      return new Blob([bytes.buffer as ArrayBuffer], { type: resolveMime(datei) });
    } catch {
      return null;
    }
  }, [usingStorage, datei]);
  ...
  if (isError || (usingStorage && signedQ.isError)) {
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-lg border border-destructive/30 bg-destructive/5 px-4 py-10 text-center text-sm text-muted-foreground">
        ...
        <p className="mt-2">{t("belege.detail.preview.loadError")}</p>
  ...
  if (!datei || !previewUrl) {
    return (
      <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border bg-muted/30 px-4 py-10 text-center text-sm text-muted-foreground">
        ...
        <p className="mt-2">{t("belege.detail.preview.none")}</p>
      </div>
    );
  }

  const mime = resolveMime(datei);
    ...
    <div className="space-y-3">
      <div className="overflow-hidden rounded-lg border border-border bg-muted/40">
        {isPdf ? (
          ...
          <Dialog>
            <DialogTrigger asChild>
              <button
                ...
                aria-label={t("belege.detail.preview.enlargeAria")}
              >
                ...
                <span className="absolute inset-0 flex items-center justify-center bg-foreground/0 opacity-0 transition-all group-hover:bg-foreground/10 group-hover:opacity-100">
                  <span className="inline-flex items-center gap-1.5 rounded-md bg-background/90 px-3 py-1.5 text-sm font-medium text-foreground shadow">
                    <Maximize2 className="size-4" /> {t("belege.detail.preview.enlarge")}
                  </span>
                </span>
              </button>
            </DialogTrigger>
          ...
          <div className="flex flex-col items-center justify-center px-4 py-10 text-center text-sm text-muted-foreground">
            <FileWarning className="size-6" />
            <p className="mt-2">
              {t("belege.detail.preview.unavailable", {
                type: mime || t("belege.detail.preview.thisFileType"),
              })}
            </p>
            <p className="text-xs">{t("belege.detail.preview.pleaseDownload")}</p>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-2">
        <span className="min-w-0 flex-1 truncate text-xs text-muted-foreground" title={filename}>
          {filename}
          {datei.size_bytes ? ` · ${(datei.size_bytes / 1024).toFixed(0)} KB` : ""}
        </span>
        {isPdf && (
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  aria-label={t("belege.detail.preview.newTab")}
                >
              ...
              <TooltipContent>{t("belege.detail.preview.newTab")}</TooltipContent>
            </Tooltip>
            <Dialog>
              <Tooltip>
                <TooltipTrigger asChild>
                  <DialogTrigger asChild>
                    <Button
                      ...
                      size="sm"
                      className="size-8 p-0"
                      aria-label={t("belege.detail.preview.zoom")}
                    >
                      <Maximize2 className="size-3.5" />
                    </Button>
                  </DialogTrigger>
                </TooltipTrigger>
                <TooltipContent>{t("belege.detail.preview.zoom")}</TooltipContent>
              </Tooltip>
      ...
      <Button asChild variant="outline" size="sm" className="w-full gap-2">
        <a href={downloadUrl ?? previewUrl} download={downloadAttr}>
          <Download className="size-4" /> {t("belege.detail.preview.download")}
        </a>
      </Button>
    </div>
  );

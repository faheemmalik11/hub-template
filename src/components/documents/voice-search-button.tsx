  ...
  return (
    <>
      <button
        ...
        aria-label={t("documents.list.nlSearch.voiceStart")}
        title={t("documents.list.nlSearch.voiceStart")}
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-sm" onClick={(e) => e.stopPropagation()}>
          <DialogHeader>
            <DialogTitle>{t("documents.list.nlSearch.voiceModalTitle")}</DialogTitle>
            {phase === "recording" && (
              <DialogDescription>{t("documents.list.nlSearch.voiceModalHint")}</DialogDescription>
            )}
          </DialogHeader>

          <div className="flex flex-col items-center gap-4 py-6">
            {phase === "error" ? (
              <p className="text-sm text-destructive">{t("documents.list.nlSearch.voiceError")}</p>
            ) : (
              <>
                <div
                  ref={orbRef}
                ...
                <p className="text-sm text-muted-foreground">
                  {phase === "transcribing"
                    ? t("documents.list.nlSearch.voiceTranscribing")
                    : phase === "requesting"
                      ? t("documents.list.nlSearch.voiceStart")
                      : t("documents.list.nlSearch.voiceRecording")}
                </p>
              </>
            )}
            ...
            {phase === "recording" && (
              <Button type="button" variant="outline" className="gap-2" onClick={stopRecording}>
                ...
                {t("documents.list.nlSearch.voiceStop")}
              </Button>
            )}
          </div>
        </DialogContent>

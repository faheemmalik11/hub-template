...
export function KnownSpellingsCard({
  ...
  const [neuerAlias, setNeuerAlias] = useState("");

  const aktive = useMemo(() => (aliasesQ.data ?? []).filter((a) => a.is_active), [aliasesQ.data]);
  ...
  return (
    <section
      className={`rounded-xl bg-card p-5 ${rahmen ? "border border-border" : ""} ${className ?? ""}`}
    >
      <div className="mb-1 flex items-center justify-between gap-2">
        ...
        {aktive.length > 0 && (
          <span className="text-xs tabular-nums text-muted-foreground">
            {t("aliases.anzahl", { count: aktive.length })}
          </span>
        )}
      </div>
      ...
      <div className="mb-3 flex items-center gap-2">
        <Input
          value={neuerAlias}
          onChange={(e) => setNeuerAlias(e.target.value)}
          placeholder={t("aliases.placeholder")}
          className="text-sm"
          onKeyDown={(e) => {
            if (e.key === "Enter" && neuerAlias.trim()) e.currentTarget.blur();
          }}
        />
        <Button
          size="sm"
          ...
          disabled={!neuerAlias.trim() || addAlias.isPending}
          onClick={() => {
            const wert = neuerAlias.trim();
            ...
            if (aktive.some((a) => folded(a.alias) === folded(wert))) {
              toast.error(t("aliases.toast.bereitsVorhanden"));
              return;
            }
            addAlias.mutate(wert, {
              onSuccess: () => {
                setNeuerAlias("");
                toast.success(t("aliases.toast.hinzugefuegt", { alias: wert }));
              },
              onError: (e) => {
                const grund = fehlerText(e);
                if (grund === "ALIAS_CLAIMED") {
                  toast.error(t("aliases.toast.beanspruchtVonAnderer", { alias: wert }));
                } else if (grund === "ALIAS_EXISTS") {
                  toast.error(t("aliases.toast.bereitsVorhanden"));
                } else {
                  toast.error(t("aliases.toast.fehlgeschlagen", { error: grund }));
                }
              },
            });
          }}
                  ...
                  <AlertDialogAction
                    onClick={() =>
                      deactivateAlias.mutate(a.id, {
                        ...
                        onError: (e) =>
                          toast.error(t("aliases.toast.fehlgeschlagen", { error: fehlerText(e) })),
                      })
                    }

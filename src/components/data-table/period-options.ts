...
import { periodToRange } from "@/lib/data/format";
 ...
 */
export const PERIOD_CUSTOM = "benutzerdefiniert";
export const PERIOD_DEFAULT = OVERVIEW_PERIOD_DEFAULT;
 ...
 */
export function usePeriodOptions(allValue: string = PERIOD_ALL) {
  const { t } = useTranslation();
  return useMemo(
    () =>
      OVERVIEW_PERIODS.map((p) => ({
        value: p === PERIOD_ALL ? allValue : p,
 ...
 */
export function periodArea(period: string, fromDate: string, toDate: string): PeriodRange {
  if (period === PERIOD_CUSTOM) return { fromDate: fromDate || null, toDate: toDate || null };
  if (isOverviewPeriod(period))
    return overviewPeriodRange(period, new Date(), { fromDate, toDate });
  return periodToRange(period, fromDate, toDate);
}

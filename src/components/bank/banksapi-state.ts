 ...
 *
 * Three distinct states, not two. The badge that used to show this read `banksapi_provider_id`
 ...
 * is on the feed is `connection_id` + `banksapi_product_id`.
 *
...
export function banksapiState(a: BankAccount): BanksapiState {
  if (a.connection_id && a.banksapi_product_id) return "connected";
  if (a.banksapi_provider_id || a.provider_id) return "linkable";
  return "none";
}

...
export function VolumeChart({
  ...
  const config = useMemo(
    () =>
      ({
        incoming: { label: t("home.money.eingang"), color: "var(--color-chart-1)" },
        outgoing: { label: t("home.money.ausgang"), color: "var(--color-chart-2)" },
      }) satisfies ChartConfig,
    [t],
  );
  const invoicesQ = useOverviewInvoices(fromDate, toDate);
  ...
  const proTag = useMemo(() => {
    ...
    return Number.isFinite(days) && days <= 62;
  }, [fromDate, toDate]);
  ...
  const data = useMemo(() => {
    ...
    const bucket = (iso: string | null | undefined) => {
      if (!iso) return null;
      const k = key(iso);
      if (!buckets.has(k)) buckets.set(k, { incoming: 0, outgoing: 0 });
      return buckets.get(k)!;
    };
    for (const r of invoices) {
      const b = bucket(r.document_date);
      if (b) b.incoming += r.amount_gross ?? 0;
    }
    for (const r of outgoing) {
      ...
      if (fromDate && r.invoice_date && r.invoice_date < fromDate) continue;
      if (toDate && r.invoice_date && r.invoice_date > toDate) continue;
  ...
  }, [invoices, outgoing, fromDate, toDate, proTag]);

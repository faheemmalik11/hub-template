...
/**
 * The alert surfaces of the notification core (portable, like the bell): a capped strip for a
 * dashboard and a full list for the notifications page. Both are presentational; the host
 * supplies translated items, already filtered to count > 0.
 *
 * ONE indicator per row: the leading icon, which an item can override to show its own kind (a
 * personal reminder is a message, not a warning triangle). The strip is EXCEPTION-ONLY: with
 * nothing open it renders nothing at all, because a permanent "all clear" box is how a reader
 * learns to skip the spot where alerts appear.
 */
...
/**
 * Severity as an icon, not a coloured dot. A dot says "something", an icon says WHAT KIND of
 * something, and it still reads for anyone who cannot separate red from amber. Static on purpose:
 * the previous warn dot pulsed its opacity to zero for half of every cycle, so those rows spent
 * half their life with no indicator visible at all.
 */
...
export function AlertStrip({
  ...
  return (
    ...
    <div className={cn("flex w-fit max-w-full flex-col items-stretch gap-1.5", className)}>
        ...
        return (
          ...
          >
            ...
            >
              ...
              {/* The sentence already carries the number, so no separate count sits beside it.
                  It WRAPS rather than truncates: the half that says what to do is the half a
      ...
      {/* Only when something is actually hidden. With three or fewer alerts the strip already
          shows all of them, so the link would point at a list the reader can see in full. */}
      {rest > 0 && seeAllTo && (
        <Link
          to={seeAllTo}
...
/** The full list for the notifications page: every open item, worst first, one row each.
 *  Acknowledged items stay listed but read as done: dimmed, marked, no motion. */

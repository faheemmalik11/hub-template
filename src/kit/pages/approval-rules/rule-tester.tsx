...
export interface TesterQuery extends RuleQuery {
  amountText: string;
}
...
export function ScenarioForm({
  ...
  draft: TesterQuery;
  onDraftChange: (next: TesterQuery) => void;
  onCheck: () => void;
  onReset: () => void;
    ...
    <section className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
      ...
      >
        ...
        <div className="flex w-36 shrink-0 flex-col gap-1.5">
          <Label
            htmlFor="approval-scenario-amount"
            className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground"
          >
            {labels.tester.amount}
          </Label>
          <Input
            id="approval-scenario-amount"
            inputMode="decimal"
            className="tabular-nums"
            value={draft.amountText}
            onChange={(event) => onDraftChange({ ...draft, amountText: event.target.value })}
          />
        </div>

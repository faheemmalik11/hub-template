 ...
 */
export function gesellschaftSchema(t: (key: string) => string) {
  return z.object({
    code: z
      .string()
      .trim()
      .min(1, t("gesellschaften.validierung.code"))
      .max(16, t("gesellschaften.validierung.codeLaenge"))
      .regex(/^[A-Za-z0-9]+$/, t("gesellschaften.validierung.codeFormat")),
    name: z.string().trim().min(1, t("gesellschaften.validierung.name")),
  });
}

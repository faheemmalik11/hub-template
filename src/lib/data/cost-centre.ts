...
import { useProperties, usePropertyCompanies } from "@/data/properties";
...
interface CostCentreMasterData {
  companies: { id: string; code: string; overhead_cost_centre: number | null }[];
...
export function resolveCostCentre(
  ...
  masterData: CostCentreMasterData,
): CostCentre | null {
  const company = masterData.companies.find((g) => g.code === input.companyCode) ?? null;
  if (!company) return null;

  if (input.overhead) {
    return {
      number: company.overhead_cost_centre ?? null,
      overhead: true,
  ...
  const propId = masterData.properties.find((o) => o.code === input.propertyCode)?.id;
  if (!propId) return null;

  const link = masterData.links.find(
    (l) => l.property_id === propId && l.company_id === company.id,
  );
  return {
    number: link?.cost_centre_number ?? null,
    overhead: false,
    companyId: company.id,
    linked: !!link,
  };
 ...
 */
export function useCostCentreResolver(): {
  ready: boolean;
  resolve: (input: CostCentreInput) => CostCentre | null;
} {
  ...
  const ready =
    companiesQ.data !== undefined && propertiesQ.data !== undefined && linksQ.data !== undefined;

  return useMemo(() => {
    const masterData: CostCentreMasterData = {
    ...
    return {
      ready,
      resolve: (input: CostCentreInput) => (ready ? resolveCostCentre(input, masterData) : null),
    };
  }, [companiesQ.data, propertiesQ.data, linksQ.data, ready]);
}

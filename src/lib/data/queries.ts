...
// E-Mail des eingeloggten Nutzers (für actor in beleg_verlauf). Best effort.
 ...
 * different problems.
 */
function pflichtGrund(grund: string | null | undefined): string {
...
// Der generierte Supabase-Database-Typ ist leer → Schreib-Payloads würden auf
// `never` laufen. Schreibzugriffe gehen daher bewusst untypisiert (wie die
// Lese-Queries ihre Ergebnisse via `as unknown as` casten).
...
function invalidateMatchState(qc: ReturnType<typeof useQueryClient>) {
...
async function insertChangeHistory(
  ...
  const { error } = await sb
    ...
    .insert({ table_name: tableName, record_id: recordId, type: typ, text, data: daten, actor });
  if (error) throw error;
}
